...
@app.command()
def run(
    url: str = typer.Option(..., "--url", "-u", help="Single PoC URL or path to batch_urls.txt (one URL per line)"),
    provider: str = typer.Option("xai", "--provider", "-p", help="xai or openai"),
    api_key: str = typer.Option(..., "--api-key", "-k", help="API key (or set XAI_API_KEY / OPENAI_API_KEY env var)", envvar=["XAI_API_KEY", "OPENAI_API_KEY"]),
    model: str = typer.Option("grok-4", "--model", "-m", help="Model name (grok-4, gpt-5-turbo, etc.)"),
    ...
    # Batch or single?
    urls = []
    if Path(url).exists() and Path(url).suffix in (".txt", ""):
        urls = [line.strip() for line in Path(url).read_text().splitlines() if line.strip() and not line.strip().startswith("#")]
    ...
    with Progress(
        ...
        for i, poc_url in enumerate(urls, 1):
            progress.update(task, description=f"[bold]Processing {i}/{len(urls)} → [yellow]{poc_url[:60]}...[/]")
